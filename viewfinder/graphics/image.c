...
static void helium_image_draw_bgr888(const graph_image_t* image);
...
static void helium_image_draw(uint32_t format, const graph_image_t* image)
{
    ...
    switch (format)
    {
        case AIPL_COLOR_BGR888:
            helium_image_draw_bgr888(image);
            break;
...
static void helium_image_draw_bgr888(const graph_image_t* image)
{
    ...
    uint8_t* framebuffer = (uint8_t*)disp_inactive_buffer();
    uint32_t frbuff_pitch = RTE_PANEL_HACTIVE_TIME;
    uint32_t frbuff_width = RTE_PANEL_HACTIVE_TIME;
    uint32_t frbuff_height = RTE_PANEL_VACTIVE_LINE;
...
{
    const uint8_t* src_ptr = image->image;
    uint16_t* framebuffer = (uint16_t*)disp_inactive_buffer();
    uint32_t frbuff_pitch = RTE_PANEL_HACTIVE_TIME;
    uint32_t frbuff_width = RTE_PANEL_HACTIVE_TIME;
    uint32_t frbuff_height = RTE_PANEL_VACTIVE_LINE;
    ...
    for (uint32_t i = 0; i < image->height && image->y + i < frbuff_height; i += 2)
    {
        ...
        while (cnt > 0)
        {
            ...
            int16x8_t c = vsubq(vreinterpretq_s16(y), 16);
            int16x8_t d = vsubq(vreinterpretq_s16(u), 128);
            int16x8_t e = vsubq(vreinterpretq_s16(v), 128);
            ...
            int32x4_t r0 = vmulq(c0, 298);
            int32x4_t r1 = vmulq(c1, 298);
            r0 = vmlaq(r0, e0, 409);
            r1 = vmlaq(r1, e1, 409);
            ...
            int32x4_t g0 = vmulq(c0, 298);
            int32x4_t g1 = vmulq(c1, 298);
            g0 = vmlaq(g0, d0, -100);
            g1 = vmlaq(g1, d1, -100);
            g0 = vmlaq(g0, e0, -208);
            g1 = vmlaq(g1, e1, -208);
            ...
            int32x4_t b0 = vmulq(c0, 298);
            int32x4_t b1 = vmulq(c1, 298);
            b0 = vmlaq(b0, d0, 516);
            b1 = vmlaq(b1, d1, 516);
            ...
            c = vsubq(vreinterpretq_s16(y), 16);
            c0 = vmovltq(c);
            c1 = vmovlbq(c);
