 ...
 * @brief    Project-local, editable ISP calibration data for the MT9M114 sensor.
 *
 *           The Ensemble pack ships default MT9M114 ISP tuning in
 *           Alif_CMSIS/Source/mt9m114_isp_param.c, whose header states the
 ...
 *           symbol link error.
 *
 *           Instead, this file keeps a tunable copy of the calibration here and
 *           copies it into the pack's global `calibration_data` at runtime, via
 *           isp_apply_calibration(), before the ISP is initialized. This makes
 *           the tuning survive pack updates and keeps all editable calibration
 *           in one place.
 *
 *           Starting point below is an exact copy of the pack 2.2.0 baseline.
 *           Tune the values here (WB gains, CCM matrices, gamma, black level,
 *           etc.) and rebuild; no pack files are modified.
 ******************************************************************************/

#include "RTE_Components.h"
#include "RTE_Device.h"

#if defined(RTE_Drivers_CAMERA_SENSOR_MT9M114) && defined(RTE_ISP) && (RTE_ISP == 1)

#include "isp_param.h"
#include "isp_calibration.h"

/* Editable calibration. Applied over the pack default at runtime. */
static const ISP_CALIB_DATA_S mt9m114_calibration = {
    .modules = {
        .autoRoute = {
...
void isp_apply_calibration(void) {
    /* Overwrite the pack default with the project-local calibration above,
     * before the ISP reads it during initialization. */
    calibration_data = mt9m114_calibration;
}

#else /* ISP + MT9M114 not both enabled */

void isp_apply_calibration(void) {
    /* Nothing to do: ISP disabled or non-MT9M114 sensor selected. */
}
