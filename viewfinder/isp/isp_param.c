 ...
 * @date     2026-04-15
 * @brief    User-configurable ISP calibration and pipeline settings.
 *           This file provides application-level ISP configuration. Users can
 *           modify calibration data (WBM, WB, CCM, DMSC), port attributes
 *           (sensor rectangle, crop), and channel attributes (output format,
 *           resolution) to match their specific sensor and application requirements.
 ******************************************************************************/
...
/* ---------------------------------------------------------------------------
 * ISP Calibration Data
 * Contains per-module calibration settings. Modules are conditionally
 * compiled based on RTE_ISP_*_MODULE flags in RTE_Device.h.
 * --------------------------------------------------------------------------- */
    ...
    .modules = {
		...
		.aem = {
			...
			.blockWin = {
				.hOffs = 0,
				.vOffs = 0,
				.hSize = RTE_ISP_SENSOR_INPUT_WIDTH,
				.vSize = RTE_ISP_SENSOR_INPUT_HEIGHT,
			},
		},
 ...
 *   Configure RTE_ISP_SENSOR_INPUT_WIDTH / RTE_ISP_SENSOR_INPUT_HEIGHT in RTE_Device.h
 *   to match your sensor (defaults to MT9M114: 1280x960).
 *   Note: snsRect.width and snsRect.height are further overridden at runtime
 *   by the camera sensor driver's reported dimensions.
 *
 * outFormRect: ISP output crop window.
 *   Configure RTE_ISP_CROP_TOP/LEFT/WIDTH/HEIGHT in RTE_Device.h for the desired window.
 *   When set to full sensor dimensions, cropping is effectively disabled.
 * --------------------------------------------------------------------------- */
